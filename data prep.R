...
grid=expand.grid(depth=10,nb_split=c(10),shrinkage=c(.001,.01))
#x=grid[1,]

results <- apply(grid,1,function(x){
  ...
  length(target)
  glm_1 <- glmnet(x=train_matrix,y = target,family="gaussian",alpha=0.5,standardize=F,nlambda = 100)
  ...
  pred_1 <- predict(gbm_1,train_db[sample_train,setdiff(names(train_db),"SalePrice")])
  pred_2 <- predict(glm_1,valid_matrix)
  obs_1 <- train_db[sample_train,"logSalePrice"]
  
  nm_gbm=paste0("pred_gbm_",i)
  print(nm_gbm)
  predictions_table=cbind(predictions_table,x=predict(gbm_1,test_db))
  setnames(predictions_table,"x",nm_gbm)
  
  nm_glm=paste0("pred_glm_",i)
  lambda_predictions=data.frame(x=predict(glm_1,newx = valid_matrix))
  compute_RMSE <- function(x){
    sqrt(sum((x-obs_1)^2))
  }
  sapply(lambda_predictions,compute_RMSE)
  
  predictions_table=cbind(predictions_table,data.frame(x=predict(glm_1,newx = test_matrix,s = tail(glm_1$lambda,1))))
...
iter=c(depth=depth,shrinkage=shrinkage,bag.fraction=.3,n.minobsinnode=n.minobsinnode,n.trees=n.trees,nb_split=nb_split,RMSE_glm=RMSE_glm,RMSE_gbm=RMSE_gbm)
print(iter)
...
predictors_gbm <- grep(pattern = "pred_gbm_",x = names(results))
prediction <- rowSums(results[,predictors])/length(predictors)
prediction_gbm <- rowSums(results[,predictors_gbm])/length(predictors_gbm)

plot(prediction,ylim=c(11,13))

test_db$predict=exp(prediction)
test_db$predict_gbm=exp(prediction_gbm)

plot(test_db$predict,ylim=c(10,12))

to_submit=test_db[,c("Id","predict")]
setnames(to_submit,"predict","SalePrice")
write_csv(to_submit,path = "submit_GBM10000GLM.csv")

to_submit=test_db[,c("Id","predict_gbm")]
setnames(to_submit,"predict_gbm","SalePrice")
write_csv(to_submit,path = "submit_onlyGBM10000.csv")
